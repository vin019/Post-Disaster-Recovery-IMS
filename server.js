...
app.post('/api/login', (req, res) => {
    ...
    query('SELECT * FROM users WHERE email = ?', [username], (err, results) => {
        ...
        bcrypt.compare(password, user.password_hash, (e, match) => {
            ...
            if (status !== 'Active') return res.status(403).json({ error: 'Account is pending approval or suspended.' });
...
app.get('/api/users', (req, res) => {
    ...
    // Added status, verified, last_active
    query('SELECT id, surname, first_name, middle_initial, email, role, contact_number, age, purok, status, verified, last_active FROM users ORDER BY id DESC', [], (err, rows) => {
...
app.post('/api/users', (req, res) => {
    ...
    bcrypt.hash(password, 10, (err, hash) => {
        ...
        // Default status: 'Pending' for everyone except maybe first admin (handled elsewhere) or if specific logic needed.
        // For now, let's make officials/viewers 'Pending' by default in DB, but we can enforce it here too.
        query('INSERT INTO users (surname, first_name, middle_initial, email, password_hash, role, contact_number, age, purok, household_head, is_head, status, verified) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
            [surname, first_name, middle_initial, email, hash, role || 'viewer', contact_number, age, purok, household_head || null, is_head || 0, 'Pending', 0], function (err, result) {
                if (err) return res.status(500).json({ error: err.message });
                logAction('System', 'Created User', `User: ${email} (Pending)`);
...
app.post('/api/users/approve/:id', (req, res) => {
    ...
    let sql = "UPDATE users SET status = 'Active', verified = 1 WHERE id = ?";
    let params = [req.params.id];

    if (role) {
        sql = "UPDATE users SET status = 'Active', verified = 1, role = ? WHERE id = ?";
        params = [role, req.params.id];
    }

    query(sql, params, function (err, result) {
        if (err) return res.status(500).json({ error: err.message });
        logAction(adminName || 'Admin', 'Approved User', `User ID: ${req.params.id} (Role: ${role || 'Unchanged'})`);
...
app.get('/api/households', (req, res) => {
    query('SELECT * FROM households', [], (err, rows) => {
        ...
        const parsed = rows.map(r => ({
            ...r,
            familyMembers: (typeof r.family_members === 'string' ? JSON.parse(r.family_members || '[]') : r.family_members) || []
        }));
        res.json(parsed);
    });
});
